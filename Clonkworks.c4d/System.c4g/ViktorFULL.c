...
Bisher sind folgende Funktionen vorhanden:
  int   GetViewRange(object pObj)
  int   GetFoW(int iPlayer, int iFunc)
...
global func GetViewRange(object pObj) { return(GetObjectVal("PlrViewRange",,pObj)); }

global func GetFoW(int iPlayer, int iFunc) { var iAsk = "ForceFogOfWar"; if(iFunc) iAsk = "FogOfWar"; return(GetPlayerVal(iAsk,0,iPlayer)); }
...
global func FxIntMorphingStart(pTarget, iEffectNumber, fTemp, idMorphTo)
{
  ...
  SetPortrait("random",pTarget,GetID(pTarget));
...
global func FxIntMorphingStop(pTarget, iEffectNumber, iReason, fTemp)
{
  ...
  SetPortrait(EffectVar(6,pTarget,iEffectNumber),pTarget,EffectVar(7,pTarget,iEffectNumber));
